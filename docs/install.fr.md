...
| `PAYSIM_PAYZEN_BRAND` | Marque Lyra du trafic arrivant par les routes du protocole : `payzen` (défaut), `systempay`, `sogecommerce`, `scellius`, `lyra`. Ces routes ne transportent pas de marque — chez Lyra c'est l'hôte qui la désigne, et Paysim n'en a qu'un. L'API de contrôle, elle, la prend dans le corps JSON : une instance peut donc héberger plusieurs intégrations. Une valeur inconnue empêche le démarrage. |
